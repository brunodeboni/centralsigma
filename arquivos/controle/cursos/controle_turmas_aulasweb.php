...
<body>
<h1 class="titulo" style="text-align:center; font-size: 30px;">Aulasweb</h1>
<div>
	<h1 class="titulo">Controle de Turmas Aulasweb</h1>
	<table>
		<tr class="firstline">
			<td><b>Curso</b>
			<td><b>Turno</b>
...
foreach ($resultado as $res) {
	...
	echo "<tr class=\"$class\">
				...
				<td>$turno	
				<td>$periodo
				<td><a href=\"email_aulasweb.php?turma=".$id_turma."\">$id_turma</a>
				<td>$status_turma
				<td><a href=\"controle_inscritos_aulasweb.php?turma=".$id_turma."&curso=".$id_curso."&turno=".$turno." \">$inscritos</a>
	";
}
...
<br>
<br>
<a class="mostrar" href="email_all_aulasweb.php">Enviar e-mail para todos os alunos</a><br>
<a class="mostrar" href="sms_all_aulasweb.php">Enviar SMS para todos os alunos</a><br>
...
echo "<br><br><b>Total inscritos Aulasweb:</b> ".$total_inscritos."<br>";

//Total de cadastrados
$sql2 = "SELECT count(id) as cadastrados FROM cwk_users";
$query2 = $db_aw->query($sql2);
$resultado2 = $query2->fetchAll();

foreach ($resultado2 as $result2) {
	$total_cadastrados = $result2['cadastrados'];
}
echo "<b>Total cadastrados Aulasweb:</b> ".$total_cadastrados."<br><br>";

?>
<div>
	<h1 class="titulo">Cadastrados Aulasweb</h1>
	<table>
		<tr class="firstline">
			<td><b>Nome</b>
			<td><b>Empresa</b>
			...
			<td><b>Origem</b>
<?php 
$sqll = "select nome, empresa, cargo, email, celular, como_conheceu	from cwk_users";
$queryy = $db_aw->query($sqll);
$resultadoo = $queryy->fetchAll();

$rowq = true; //classe das cores das linhas

foreach ($resultadoo as $ress) {
...
$query2 = $db_aw->query($sql2);
$resultadoo2 = $query2->fetchAll();

$rowq = true; //classe das cores das linhas

foreach ($resultadoo2 as $res2) {
...
<br>
<a class="mostrar" href="relatorio_cursos_aulasweb.php" target="_blank">Gerar Relat&oacute;rio em PDF</a>
<br>
</div>
<br>
<br>
...
foreach ($resultado1 as $res) {
	...
	echo "
		<tr class=\"$class\">
			<td><a href=\"inscritos_dia_aulasweb.php?dia=".$dia."\">$dia</a>
			<td>$num_inscritos
			<td>$num_users
		";
}
	echo "</table>";
	echo '<br><a class="mostrar" href="relatorio_data_aulasweb.php" target="_blank">Gerar Relat&oacute;rio em PDF</a><br>';
?>
</div>
<br>
<div>
...
foreach ($resultado as $res) {
	...
	for ($mes = 1; $mes <= 12; $mes++) { //loop de usuários por mês
        ...
        $query_pes = $db_aw->prepare($pesq1);
        $query_pes->execute(array(
        	':como_conheceu' => $como_conheceu,
        	':mes' => $mes,
        	':ano_pesq' => $ano_pesq
        ));
        $resultado = $query_pes->fetchAll();
		foreach ($resultado as $result) {
	...
	$pesq2 = "SELECT count(como_conheceu) as total_ano
		...
		WHERE como_conheceu in (:como_conheceu)
		AND year(dh_inscrito)=:ano_pesq";
	$query_pes2 = $db_aw->prepare($pesq2);
	$query_pes2->execute(array(
		':como_conheceu' => $como_conheceu,
		':ano_pesq' => $ano_pesq
	));
	$resu = $query_pes2->fetchAll();
	foreach ($resu as $r) {
		echo "<td>".$r['total_ano'];
	}	
}
...
<br>
<a class="mostrar" href="relatorio_conheceu_aulasweb.php?ano=<?php echo $ano_pesq?>" target="_blank">Gerar Relat&oacute;rio em PDF</a>
<br>
</div>
...
if (isset ($_POST['uf'])) {
	...
	echo '<a class="mostrar" href="relatorio_uf_aulasweb.php?uf='.$uf.'" target="_blank">Gerar Relat&oacute;rio em PDF</a><br>';
	...
	$consulta = $db_aw->query($cons);
	$dadoss = $consulta->fetchAll();
