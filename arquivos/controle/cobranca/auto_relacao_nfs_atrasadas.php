...
foreach ($gama as $id_nota => $smth) {
	if ($smth[0] != null) {
		...
		//Se a data da parcela com vencimento pendente (primeira) já passou 
		if (strtotime($alpha[$id_nota][0]) < strtotime(date('d-m-Y'))) {
			//Coloca no array beta a data que está pendente
...
$assunto = 'Relação de NFs pendentes em '.date('d/m/Y');
			
$msg = 'Marisabel,

Esta é uma notificação automática do relatório das Notas Fiscais com pagamento ainda pendente em '.date("d/m/Y").':';
